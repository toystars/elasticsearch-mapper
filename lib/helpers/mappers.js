...
var inspector = require('util'),
  _ = require('lodash'),
  schemaGen = require('mongo-schema-gen');
...
/**
 * transformField
 * @summary Transform schema field object from mongo-schema-gen package to bare-bone schema field object
 * @param {Object} value - schema field object to analyze
 * @param {string} indexName - name of index to check key log against
 * @param {Object} keyLog - Object containing all unique keys in all the index registered to prevent type mismatch
 * @return {Object} mapping field object
 */
var transformField = function (value, indexName, keyLog) {
  var object = {};
  if (typeof value.type === 'string') {
    ...
    if (typeof newValue === 'string') {
      ...
      object.type = 'nested';
      object.properties = analyzeCollectionSchema(newValue, indexName, keyLog);
    }
  } else if (typeof value.type === 'object') {
    object.type = 'object';
    object.properties = analyzeCollectionSchema(value.type, indexName, keyLog);
  }
  return object;
};
 ...
 * @summary analyze and create bare-bone mapping with only field type from schema generated from collection
 * @param {Object} schema - schema to analyze
 * @param {string} indexName - name of index to check key log against
 * @param {Object} keyLog - Object containing all unique keys in all the index registered to prevent type mismatch
 * @return {Object} mapping
 */
var analyzeCollectionSchema = function (schema, indexName, keyLog) {

  // transform schema from schema structure to bare-bone JSON-Type structure
  var object = {};
  for (var key in schema) {
    if (schema.hasOwnProperty(key) && key !== '_id') {
      var typeObject = transformField(schema[key], indexName, keyLog);
      var keyTypeValidity = getKeyTypeValidity(typeObject.type, key, indexName, keyLog);
      if (keyTypeValidity.valid) {
        object[key] = typeObject;
      } else {
...
var configureDefaultMap = function (mapping) {
  ...
  _.each(keys, function (key) {
    ...
    if (step.type === 'string') {
      step.analyzer = 'edgeNGram_analyzer';
    ...
    if (step.type === 'string') {
      ...
      if (mapConfig) {
        if (!mapConfig.tokenize) {
          ...
          step.analyzer = mapConfig.index ? settings.analysis.analyzer.hasOwnProperty(mapConfig.index) ? mapConfig.index : 'edgeNGram_analyzer' : 'edgeNGram_analyzer';
...
module.exports = {
  ...
  /**
   * createMappingsFromCollection
   * @summary create type mapping from a mongo collection
   * @param {Object} arg - argument object containing all necessary values
   * @param {Function} callBack - function to invoke with mapping result
   * @return {Object} mapping object representing elasticsearch type
   */
  createMappingsFromCollection: function (arg, callBack) {

    var settings = arg.settings;
    var mongoUrl = arg.config.mongoUrl;
    var collectionName = arg.config.collectionName;
    var mapperConfig = arg.config.config;
    var indexName = arg.indexName;
    var keyLog = arg.log;

    schemaGen.connect(mongoUrl, function (db) {
      schemaGen.getSchema(collectionName, function (schema) {
        var newSchema = {};
        ...
        var mapping = analyzeCollectionSchema(newSchema, indexName, keyLog);
        var finalMapping = {
          _all: {
            enabled: false
          },
